...
class yolov11_heatmap:
    ...
    def draw_detections(self, box, cls_id, conf, img):
        ...
        (tw, th), baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.3, 1)
        ty = max(ymin - 5, 0)  # 默认位置在框上方
        tx = xmin

        # 如果当前文字区域与已放置的重叠，就往下挪
        while any(
                tx < ux + uw and tx + tw > ux and ty - th < uy and ty > uy - uh
                for (ux, uy, uw, uh) in self.used_label_boxes
        ):
            ty += th + baseline + 2  # 向下挪一行

        # 保存已用区域 (x, y baseline, w, h)
        self.used_label_boxes.append((tx, ty, tw, th + baseline))

        # 绘制文字
        cv2.putText(img, label, (tx, ty), cv2.FONT_HERSHEY_SIMPLEX,
                    0.3, c, 1, lineType=self.line_type)
        return img
...
if __name__ == '__main__':
    ...
    save_path = r'E:\DataSets\ceshiresult_yolo11_FASFFHead_P234_RCSOSA_wiou_bce'  # 保存结果的路径
    # 遍历所有的参数并生成热力图
