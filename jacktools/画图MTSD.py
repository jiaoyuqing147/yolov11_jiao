...
#     r'../runsTT100k130/yo ilo11-FASFFHead_P234_RCSOSA_wiou_bce_distillation/exp/results.csv',
# ]
...
def plot_recall():
    ...
    # ---- 原本的：标题在图外（保留为注释）----
    # plt.title('Recall Comparison of YOLO11n Variants on MTSD Dataset',
    #           fontsize=14, fontweight='bold')

    # ✅ 新增：把标题放到图内部左上角
    ax = plt.gca()
    ax.text(0.02, 0.95,
            'Recall Comparison of YOLO11n Variants on MTSD Dataset',
    ...
    plt.legend(fontsize=10)
    plt.tight_layout()
    outpath = os.path.join(save_dir, 'recall_comparison_MTSD.png')
    plt.savefig(outpath, dpi=dpi, bbox_inches='tight')
    plt.show()
    print(f'✅ 保存：{outpath}')
...
def plot_total_val_loss():
    ...
    # ---- 原本的：标题在图外（保留为注释）----
    # plt.title('Total Validation Loss of YOLO11n Variants on MTSD Dataset',
    #           fontsize=14, fontweight='bold')

    # ✅ 新增：把标题放到图内部左上角
    ax = plt.gca()
    ax.text(0.02, 0.95,
            'Total Validation Loss of YOLO11n Variants on MTSD Dataset',
            transform=ax.transAxes, fontsize=10, fontweight='bold',
            va='top', ha='left',
            bbox=dict(facecolor='white', alpha=0.6, edgecolor='none', pad=3))
    ...
    plt.legend(fontsize=10)
    plt.tight_layout()
    outpath = os.path.join(save_dir, 'total_val_loss_MTSD.png')
    plt.savefig(outpath, dpi=dpi, bbox_inches='tight')
    plt.show()
    print(f'✅ 保存：{outpath}')
...
if __name__ == '__main__':
    ...
    plot_metric('metrics/mAP50(B)',
                'mAP@0.5 Comparison of YOLO11n Variants on MTSD Dataset',
                'mAP@0.5', 'map50_comparison_MTSD.png', pick='max')
    plot_metric('metrics/mAP50-95(B)',
                'mAP@0.5:0.95 Comparison of YOLO11n Variants on MTSD Dataset',
                'mAP@0.5:0.95', 'map50_95_comparison_MTSD.png', pick='max')
    plot_total_val_loss()
