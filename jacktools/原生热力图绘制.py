...
def get_params():
    params = {
        #'weight': r'../runsTT100k130/yolo11_train200/exp/weights/best.pt',  # 训练出来的权重文件
        #'weight': r'../runsTT100k130/yolo11-OECSOSAInterleave_train200/exp/weights/best.pt',  # 训练出来的权重文件
        'weight': r'../runsTT100k130/yolo11-FASFFHead_P234_train200/exp/weights/best.pt',  # 训练出来的权重文件
        #'cfg': r'../ultralytics/cfg/models/11/yolo11.yaml',  # 训练权重对应的yaml配置文件

        'cfg': r'../ultralytics/cfg/models/11/yolo11-FASFFHead_P234.yaml',  # 训练权重对应的yaml配置文件
        'device': 'cuda:0',
        'method': 'GradCAM',
        # GradCAMPlusPlus, GradCAM, XGradCAM, EigenCAM, HiResCAM, LayerCAM, RandomCAM, EigenGradCAM, 使用的热力图库文件不同的效果不一样可以多尝试
        'layer': 'model.model[19]',  # 想要检测的对应层，可以更换不同的层去检测
        'backward_type': 'class',  # class, box, all
        'conf_threshold': 0.45,  # 0.6  # 置信度阈值，有的时候你的进度条到一半就停止了就是因为没有高于此值的了
        'ratio': 0.02  # 0.02-0.1
    }
    return params
...
if __name__ == '__main__':
    ...
    model(r'F:\DataSets\resultTT100k130train\multi_model_comparenew\TopK_vis\23858.jpg',
          r'F:\DataSets\resultTT100k130train\multi_model_comparenew\TopK_vis')  # 第一个是检测的文件, 第二个是保存的路径