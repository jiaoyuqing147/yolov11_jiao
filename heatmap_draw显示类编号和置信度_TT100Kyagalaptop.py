...
class yolov11_heatmap:
    def __init__(
        ...
        # 使用我们自定义的全局 COLORS（固定 10 种鲜艳色）
        self.fixed_colors = COLORS

        # 把局部变量全挂到 self 上（和你原来一样）
    ...
    def draw_detections(self, box, cls_id, conf, img):
        ...
        cv2.putText(
            ...
            float(self.font_scale) * 0.8,    # 缩小一点更适合小目标
            color,
    ...
    def process(self, img_path, save_path):
        ...
        if self.renormalize and pred is not None and len(pred) > 0:
        ...
        if self.show_box and pred is not None and len(pred) > 0:
            for det in pred:
    ...
    def __call__(self, img_path, save_path, grad_name):
        ...
        if os.path.isdir(img_path):
            for img_path_ in os.listdir(img_path):
                ...
                self.process(
                    f'{img_path}/{img_path_}',
                    f'{save_path}/{name}_{grad_name}.{end_name}'
                )
        else:
...
if __name__ == '__main__':
    ...
    save_path = r'E:\DataSets\forpaper\ceshiTT100Kresult_yolo11'  # 保存结果的路径

    # 遍历所有的参数并生成热力图
    for each in get_params():
        model = yolov11_heatmap(**each)
        # model 第一个参数：单张图片路径或者图片文件夹路径;
        # 第二个参数：保存路径; 第三个参数：绘制热力图方法
        model(img_path, save_path, each['method'])  # 生成热力图并保存到指定路径
