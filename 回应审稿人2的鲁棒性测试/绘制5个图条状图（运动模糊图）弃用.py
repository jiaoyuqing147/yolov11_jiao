...
LABEL_FONT_SIZE = 3.3       # 类别文字大小，太挤可以改成 4.8 或 5.0
LABEL_BBOX_PAD = 0.0       # 标签背景框内边距，越小越贴文字
...
VIEW_TAG = "pixelcrop" if USE_CROP else "full"
...
I4_ID = CLASS_NAMES.index("i4")
I5_ID = CLASS_NAMES.index("i5")

# i4 原来可能是黄色，换成紫红色，避免和背景/浅色区域混在一起
CLASS_COLOR_MAP[I4_ID] = "#D000FF"

# i5 原来可能是蓝色，和底部颜色太像，换成橙红色
CLASS_COLOR_MAP[I5_ID] = "#FF4500"
# pb 原来颜色接近青色背景，这里单独改成橙色，增强对比度
...
def get_crop_region(img_bgr):
    ...
    if x_max <= x_min:
        raise ValueError(
            f"Invalid crop x range: x_min={x_min}, x_max={x_max}"
        )

    if y_max <= y_min:
        raise ValueError(
            f"Invalid crop y range: y_min={y_min}, y_max={y_max}"
        )
...
# 参考雨图脚本的画法：直接使用 ax.text 的 bbox 作为标签底板。
            # 这种方式在你的雨图效果中可以让底板更贴近文字。
            ax.text(
                text_x,
                text_y,
                label,
...
# =====================================================
# 这里只保留上面的五张单图输出。
# 如果后续还想恢复拼接图，可以重新调用 save_vector_montage。
