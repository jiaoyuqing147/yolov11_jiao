"""
Draw GT and Prediction Boxes for Fog Robustness Visualization
...
CROP_LEFT_RATIO = 0.458984375      # 940 / 2048
CROP_RIGHT_RATIO = 0.384765625     # (2048 - 1260) / 2048

CROP_TOP_RATIO = 0.390625          # 800 / 2048
CROP_BOTTOM_RATIO = 0.453125       # (2048 - 1120) / 2048
...
if __name__ == "__main__":
    ...
    if fog_img_path is None:
        raise FileNotFoundError(
            f"Fog image not found: {IMAGE_ID}"
        )
    ...
    if fog_img is None:
        raise RuntimeError(
            f"Failed to read fog image: {fog_img_path}"
        )

    print(f"Original image shape before resize : {original_img.shape}")
    print(f"Fog image shape before resize      : {fog_img.shape}")

    # =====================================================
    # 统一 Fog 图尺寸到 Original 图尺寸
    # =====================================================
    # 一般 val_fog 和 original 尺寸应该一致。
    # 这里保留 resize 检查，防止尺寸不一致导致框和字体比例异常。
    # YOLO 标签是归一化坐标，因此 resize 后框的位置仍然正确。

    if fog_img.shape[:2] != original_img.shape[:2]:

        print(
            f"[Info] Resize fog image from {fog_img.shape[:2]} "
    ...
    print(f"Fog image shape after resize       : {fog_img.shape}")
    ...
    yolo_fog_boxes = read_pred_labels(
        YOLO_FOG_LABEL_DIR /
        f"{IMAGE_ID}.txt",
        conf_thresh=CONF_THRESH
    )
    ...
    ecafa_fog_boxes = read_pred_labels(
        ECAFA_FOG_LABEL_DIR /
        f"{IMAGE_ID}.txt",
        conf_thresh=CONF_THRESH
    )
    ...
    yolo_fog_boxes_before = yolo_fog_boxes
    ecafa_original_boxes_before = ecafa_original_boxes
    ecafa_fog_boxes_before = ecafa_fog_boxes
    ...
    yolo_fog_boxes = remove_heavily_overlapped_boxes(
        yolo_fog_boxes,
        fog_img,
        iou_thresh=VIS_NMS_IOU_THRESH
    )
    ...
    ecafa_fog_boxes = remove_heavily_overlapped_boxes(
        ecafa_fog_boxes,
        fog_img,
        iou_thresh=VIS_NMS_IOU_THRESH
    )
    ...
    panel_items = [
        ...
        },
        {
            ...
            "boxes": ecafa_fog_boxes,
            "mode": "pred",
            "title": "ECAFA-YOLO Fog",
        },
    ]

    montage_base = (
        OUTPUT_DIR /
        f"{IMAGE_ID}_5cols_montage_fog_{VIEW_TAG}_vector_class_color"
    )
