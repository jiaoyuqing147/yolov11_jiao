...
def LSKNET_Tiny(factor):
    model = LSKNet(factor=factor, depths=[2, 2, 2, 2])
    return model


def LSKNET_Large(factor):
    model = LSKNet(factor=factor, depths=[3, 4, 6, 3])
    return model


if __name__ == '__main__':
    model = LSKNET_Large(factor=0.5)
