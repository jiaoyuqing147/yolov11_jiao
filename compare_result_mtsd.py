...
# ========== 配置 ==========
IMAGES_DIR = Path(r"E:\DataSets\MTSD\yolo54\images\test")  # 图像目录（用于拿到宽高）
GT_LABELS = Path(r"E:\DataSets\MTSD\yolo54\labels\test")  # GT 标签目录（YOLO txt，无 conf）
PRED_A = Path(r"E:\DataSets\MTSDresult\yolo11_train")  # 模型A 预测标签目录（有 conf）
PRED_B = Path(r"E:\DataSets\MTSDresult\yolo11_WIOU+BCELoss_train")  # 模型B 预测标签目录（有 conf）

OUT_CSV = Path(r"E:\DataSets\MTSDresult\AB_compare_per_image.csv")
COPY_TOPK = 10  # A 明显优于 B 的前 K 张（按 ΔF1 排序），0 表示不拷贝
COPY_BOTTOMK = 10  # B 明显优于 A 的前 K 张（按 -ΔF1 排序）
COPY_DIR_TOP = Path(r"E:\DataSets\MTSDresult\TOP_A_over_B")
COPY_DIR_BOT = Path(r"E:\DataSets\MTSDresult\TOP_B_over_A")
